...
import { Sparkles, Map, Compass, Mic, LogIn, LogOut, Globe, ChevronDown, Menu, X } from 'lucide-react';
    ...
    <>
      <header className="sticky top-0 z-50 bg-[#DCE9E4] border-b border-[#B8D4CB]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center space-x-2.5 sm:space-x-3.5 group shrink-0">
            <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg bg-[#FFFFFF] border border-[#B8D4CB] overflow-hidden flex items-center justify-center group-hover:border-[#2F6E5D] transition-colors shadow-sm shrink-0">
              <img src="/images/logo.png" alt="Dharohar Setu Logo" className="w-full h-full object-cover" />
            </div>
            <div>
              <span className="font-serif text-lg sm:text-2xl font-semibold tracking-tight text-[#2A2420] block leading-none">
                Dharohar Setu
              </span>
              <span className="text-[10px] sm:text-xs text-[#2F6E5D] font-sans tracking-wide font-medium mt-0.5 sm:mt-1 inline-block">
                {t('brandSubtitle')}
              </span>
            </div>
          </Link>
          ...
          <div className="flex lg:hidden items-center space-x-2">
            {/* Capture CTA on mobile */}
            <Link
              href="/capture"
              className="inline-flex items-center space-x-1.5 px-3 py-2 rounded-md bg-[#C97A3D] text-[#FAF7F1] font-sans font-medium text-xs hover:bg-[#B86B30] transition-colors"
            >
            ...
            <button
              ...
              className="p-2 rounded-md text-[#2A2420]/80 hover:text-[#2F6E5D] hover:bg-[#FFFFFF]/60 transition-colors"
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          {/* Backdrop */}
          <div
            className="absolute inset-0 bg-[#2A2420]/40 backdrop-blur-sm"
            onClick={() => setMobileMenuOpen(false)}
          />

          {/* Drawer */}
          <div className="absolute top-0 right-0 h-full w-72 max-w-[85vw] bg-[#FAF7F1] shadow-2xl flex flex-col overflow-y-auto">
            {/* Drawer Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E4DDD0] bg-[#DCE9E4]">
              <div className="flex items-center space-x-2.5">
                <div className="w-8 h-8 rounded bg-[#FFFFFF] border border-[#B8D4CB] overflow-hidden flex items-center justify-center">
                  <img src="/images/logo.png" alt="Logo" className="w-full h-full object-cover" />
                </div>
                <span className="font-serif text-base font-semibold text-[#2A2420]">Dharohar Setu</span>
              </div>
              <button
                onClick={() => setMobileMenuOpen(false)}
                className="p-1.5 rounded text-[#2A2420]/60 hover:text-[#2A2420] hover:bg-[#E4DDD0] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-sans transition-colors ${
                      isActive
                        ? 'bg-[#2F6E5D]/10 text-[#2F6E5D] font-semibold border border-[#2F6E5D]/20'
                        : 'text-[#2A2420]/85 hover:bg-[#E4DDD0]/60 hover:text-[#2F6E5D]'
                    }`}
                  >
                    {link.icon && <link.icon className="w-4 h-4 shrink-0 text-[#C97A3D]" />}
                    {!link.icon && <div className="w-4 h-4 shrink-0 rounded-full bg-[#C97A3D]/20 border border-[#C97A3D]/30" />}
            ...
            {/* Language Switcher */}
            <div className="px-3 pb-3 border-t border-[#E4DDD0] pt-3">
              <p className="text-[10px] uppercase tracking-wider font-semibold text-[#2A2420]/50 px-4 mb-2">Language</p>
              <div className="grid grid-cols-2 gap-1.5 px-1">
                {LANGUAGE_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded text-xs flex items-center justify-between transition-colors ${
                      language === opt.code
                        ? 'bg-[#2F6E5D]/10 text-[#2F6E5D] font-semibold border border-[#2F6E5D]/20'
                  ...
                  >
                    <span>{opt.nativeName}</span>
                    <span className="text-[10px] text-[#2A2420]/40 font-mono uppercase">{opt.code}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Auth */}
            <div className="px-3 pb-5 pt-3 border-t border-[#E4DDD0]">
              {user ? (
                <div className="space-y-2">
                  <div className="px-4 py-2 rounded-lg bg-[#E4DDD0]/50 border border-[#E4DDD0]">
                    <p className="text-xs font-semibold text-[#2A2420]">{user.displayName}</p>
                    {user.email && <p className="text-[10px] text-[#2A2420]/55 font-mono truncate">{user.email}</p>}
                    <span className={`mt-1 inline-block text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-wider font-semibold ${getRoleBadge(user.role)}`}>
                      {user.role}
                    </span>
                  </div>
                  <button
                    onClick={() => { setMobileMenuOpen(false); logout(); }}
                    className="w-full flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg text-xs font-medium text-[#B54A3A] bg-[#B54A3A]/10 border border-[#B54A3A]/30 hover:bg-[#B54A3A]/15 transition-colors"
                  >
                ...
                <Link
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg text-sm font-medium text-[#2A2420]/85 bg-[#FFFFFF] border border-[#B8D4CB] hover:border-[#2F6E5D] hover:text-[#2F6E5D] transition-colors w-full"
                >
